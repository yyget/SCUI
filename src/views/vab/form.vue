		...
		<el-row :gutter="15">
			<el-col :lg="6">
				...
				>
					<el-timeline>
						<el-timeline-item
							timestamp="两者数据并无顺序要求,只是获取表单值时需要自行设置loading"
						...
						>
						<el-timeline-item timestamp="合并form对象和表单值"
							>3.给表单赋值</el-timeline-item
						>
					</el-timeline>
					<el-alert
...
<script>
	export default {
		...
		methods: {
			...
			setConfig() {
				this.config.formItems[2].span =
					this.config.formItems[2].span == 24 ? 12 : 24
				this.config.formItems[3].span =
					this.config.formItems[3].span == 24 ? 12 : 24
			},
			save() {
